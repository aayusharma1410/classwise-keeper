...
export const uploadAllData = async () => {
  try {
    ...
    // Upload students data for Section A with photo_url field
    const studentsData = [
    ...
    // Add more students from user provided data if needed
    ...
    const { error: additionalStudentsError } = await supabase
      .from('students_section_a')
      .upsert(additionalStudents, { onConflict: 'id' });

    return {
      classCodesSuccess: true,
      sectionASuccess: !studentsError && !subjectsError && !additionalStudentsError,
      message: studentsError || subjectsError || additionalStudentsError
        ? `Error uploading some data: ${studentsError?.message || ''} ${subjectsError?.message || ''} ${additionalStudentsError?.message || ''}`
